...
- **Light Controls (Lights only, optional).** A fifth capability alongside toggle/on/off, in its own collapsed section: **brightness** (low/medium/high/full named wordings, plus a free-number wording that reads whatever percent was said), **warmth** (warm/neutral/cool named wordings, plus a free-number wording for a specific Kelvin value), and **color** (one wording that reacts to a small built-in set of color names - red, orange, yellow, green, blue, purple, pink - said anywhere inside it). Every field is optional and independent, same comma-separated convention as the phrases above. These share the rule's own success/error/not-found/unknown-room responses rather than a second set. Registered separately from toggle/on/off and isolated so that a problem here can't affect this rule's toggle/on/off, or any other rule (see CHANGELOG 2026.09.21.01/.02 for why that isolation exists).

Any of the four response fields can now hold more than one line - each line is a candidate reply, and one is picked at random every time the rule fires, so a frequently-triggered rule doesn't always say back the exact same sentence. A single-line response still works exactly as it always has.

None of the responses change what actually happens - they only override what's said back. Every optional field, including the label filter, can be cleared back to blank later through **Configure** - emptying it and saving actually removes it rather than silently keeping the old value.
...
- **Light Controls' brightness/warmth are named presets plus one free number, not a full range picker**, and color is a small fixed set of names rather than every hue - a voice phrase isn't well suited to precise slider-style input anyway, but the preset values themselves aren't user-editable yet (only the wordings that trigger them are).
